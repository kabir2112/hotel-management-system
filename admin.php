...
/*        search box css*/
    ...
    <div class="navbar">
   <!--  <p class="con-logo"> <img src="picture/1.jpg" width=40px height="30px"> </p>-->
...
<!--EXPIRED ROOM LIST-->
    ...
    $count=0;
    $con=pg_connect("host = localhost port=5432 dbname=project user=postgres password=123") or die("connection Failed");
    $query="SELECT * FROM room WHERE 'Checkout' < 'CURRENT_DATE' AND 'Status'='paid' ";
...
<!--EXPIRED EVENT LIST-->
    ...
    $count=0;
    $con=pg_connect("host = localhost port=5432 dbname=project user=postgres password=123") or die("connection Failed");
    $query="select * from events WHERE 'Checkin' < 'CURRENT_DATE'";
    $num=pg_query($con,$query);
    if(pg_num_rows($num)>0)
    {
    while($row=pg_fetch_assoc($num))
    ...
    ?>
    <META HTTP-EQUIV="Refresh" CONTENT="0; URL=http://localhost/hotel/admin.php#user"/>
    <?php
    }
    else
    {
